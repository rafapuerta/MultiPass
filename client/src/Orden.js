    ...
    function compararValor(clave, orden = 'asc') {
    return function ordenar(a, b) {
      if (!a.hasOwnProperty(clave) || !b.hasOwnProperty(clave)) {
        return 0;
      }
  
      const A = (typeof a[clave] === 'string')
        ? a[clave].toUpperCase() : a[clave];
      const B = (typeof b[clave] === 'string')
        ? b[clave].toUpperCase() : b[clave];
  ...
  console.log(personas.sort(compararValor("company")))